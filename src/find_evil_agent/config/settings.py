...
class Settings(BaseSettings):
    ...
    sift_ssh_user: str | None = "sansforensics"
    sift_ssh_key_path: str | None = None

    # Security
    allowed_evidence_paths: list[str] = Field(
        default=["/mnt/evidence/", "/workspace/", "/tmp/sift-workspace/"]  # nosec B108
    )
    ...
    def validate_provider_config(self) -> "Settings":
        ...
        if self.llm_provider == LLMProviderEnum.OPENAI and not self.openai_api_key:
            raise ValueError(
                "OPENAI_API_KEY environment variable required when using openai provider"
            )
        if self.llm_provider == LLMProviderEnum.ANTHROPIC and not self.anthropic_api_key:
            raise ValueError(
                "ANTHROPIC_API_KEY environment variable required when using anthropic provider"
            )
        return self
