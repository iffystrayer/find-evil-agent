"""MCP tool functions — `@mcp.tool()` registrations.

Split out of `mcp/server.py` in C3a so the FastMCP wiring, validation
helper, and tool definitions each live in their own module. Importing
this module registers every tool against the shared `mcp` instance.

Adding a new tool? Define it here and re-export it from
`find_evil_agent.mcp.server` so the existing
`from find_evil_agent.mcp.server import <tool>` import paths keep
working.
"""
...
from .server import mcp, _validate_evidence_path
