...
async def resume_investigation(
    session_id: str, approved: bool, format_dropdown: str, progress=gr.Progress()
) -> tuple:
    ...
    try:
        ...
        if max_iterations == 1:
            ...
            if result.success:
                ...
                tool_name = (
                    state_obj.selected_tools[0].tool_name
                    if getattr(state_obj, "selected_tools", [])
                    else "unknown"
                )
        ...
        iterations_text = (
            f"{max_iterations} iterations" if max_iterations > 1 else "single analysis"
        )
...
"""
        ...
        return (f"<p style='color: red;'>Error: {str(e)}</p>", error_msg, None)
...
async def investigate_incident(
    ...
    box_vis = (
        ...
        if "Waiting for Human Approval" in status
...
def create_app() -> gr.Blocks:
    ...
    """
    with gr.Blocks(title="Find Evil Agent - AI DFIR Automation") as demo:
            ...
            with gr.Row():
                ...
                with gr.Column(scale=1):
                    status_output = gr.Textbox(
                        label="Status", lines=12, elem_classes=["status-box"], interactive=False
                    )
        ...
        with gr.Tab("🔬 Investigative Mode"):
            ...
            with gr.Row():
                with gr.Column(scale=1):
                    ...
                    format_dropdown_inv = gr.Dropdown(
                        choices=["html", "markdown"], value="html", label="Report Format"
                    )
                    investigate_btn = gr.Button("🔍 Investigate", variant="primary", size="lg")

                with gr.Column(scale=1):
                    status_output_inv = gr.Textbox(
                        label="Status", lines=10, elem_classes=["status-box"], interactive=False
                    )
                    ...
                    with gr.Group(visible=False) as hitl_box:
                        ...
                        gr.Markdown(
                            "The agent successfully discovered autonomous leads and is waiting for cryptographic signing to proceed."
                        )
                        with gr.Row():
            ...
            provider_dropdown_inv.change(
                fn=update_models, inputs=[provider_dropdown_inv], outputs=[model_dropdown_inv]
            )
...
def launch_app(
    server_name: str = "0.0.0.0", server_port: int = 17000, share: bool = False, debug: bool = False  # nosec B104
):
