...
class TestCommandBuilderSpecification:
    ...
    def test_expected_outputs_specification(self):
        ...
        expected_outputs = {
            ...
            "failure_case": {
                "command": None,
                "error": "Invalid path or injection detected"
            }
        }
...
def mock_llm_router():
    ...
    router.route.return_value = Mock(
        success=True,
        content="volatility -f /mnt/evidence/memory.raw --profile=Win10x64 pslist"
    )
    return router
...
class TestCommandBuilderExecution:
    ...
    @pytest.mark.skipif(not BUILDER_AVAILABLE, reason="DynamicCommandBuilder not implemented")
    async def test_command_respects_tool_metadata(
        self,
        mock_llm_router,
        investigation_context
    ):
        ...
        mock_llm_router.route.return_value = Mock(
            success=True,
            content="strings /mnt/evidence/disk.dd"
        )
        strings_selection = ToolSelection(
            tool_name="strings",
            confidence=0.85,
            reason="String extraction needed"
        )
...
class TestCommandBuilderValidation:
    ...
    async def test_rejects_command_injection(self, mock_llm_router):
        ...
        mock_llm_router.route.return_value = Mock(
            success=True,
            content="strings /mnt/evidence/file.txt; rm -rf /"
        )
