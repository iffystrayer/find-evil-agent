...
class TestAnthropicProviderStructure:
    ...
    def provider(self):
        ...
        return AnthropicProvider(
            api_key="sk-ant-test-key-for-structure-tests", model_name="claude-sonnet-4-20250514"
        )

    @pytest.mark.skipif(
        not ANTHROPIC_PROVIDER_AVAILABLE, reason="AnthropicProvider not implemented yet"
    )
    ...
    def test_provider_accepts_required_parameters(self):
        ...
        provider = AnthropicProvider(
            api_key="sk-ant-test-key", model_name="claude-sonnet-4-20250514"
        )
...
class TestAnthropicProviderExecution:
    ...
    async def test_generate_simple_text(self, provider):
        ...
        response = await provider.generate(
            "Say 'Hello from Anthropic' and nothing else.", temperature=0.0
        )
...
class TestAnthropicProviderIntegration:
    ...
    def provider(self):
        ...
        return AnthropicProvider(
            api_key=api_key, model_name="claude-haiku-4-20250514", temperature=0.1, max_tokens=2048
        )
...
class TestAnthropicProviderErrorHandling:
    ...
    async def test_invalid_api_key_raises_error(self):
        ...
        provider = AnthropicProvider(
            api_key="sk-ant-invalid-key-12345", model_name="claude-haiku-4-20250514"
        )
    ...
    async def test_invalid_model_raises_error(self):
        ...
        provider = AnthropicProvider(api_key=api_key, model_name="invalid-model-xyz")
